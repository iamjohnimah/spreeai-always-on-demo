...
export default function App(){
 ...
 const [chosenTwin,setChosenTwin]=useState(person),[manualSize,setSize]=useState(''),[sizeRequest,setSizeRequest]=useState(0),[filter,setFilter]=useState('All'),[sort,setSort]=useState('featured');
 ...
 return <>
  ...
  {!product&&<>
   ...
   <section className="collection-intro"><p className="eyebrow">THE SPREEAI COLLECTION</p><h1>Always On</h1><p>Your style. Your fit. Already you.</p></section>
   <nav className="mainnav" aria-label="Collection navigation"><button className={filter==='All'?'current':''} onClick={()=>{setFilter('All');setLimit(12)}}>Always On</button>{['Outerwear','Knitwear','Bottoms','Shoes','Accessories','Dresses'].map(c=><button className={filter===c?'current':''} key={c} onClick={()=>{setFilter(c);setLimit(12)}}>{c}</button>)}</nav>
  ...
  <Dialog.Root open={!!panel} onOpenChange={v=>{if(!v)close()}}><Dialog.Portal><Dialog.Overlay className="overlay"/><Dialog.Content className={`modal ${['profile','menu'].includes(panel||'')?'drawer':''} ${['account','sizes','why','live','journey','twins','compare','stylist','saved','colors'].includes(panel||'')?'wide':''} ${panel==='account'?'account-modal':''} ${panel==='guide'?'guide-modal':''} ${panel==='checkout'?'checkout-modal':''}`} aria-describedby="modal-description"><Dialog.Close className="close" aria-label="Close dialog">×</Dialog.Close>
   ...
   {panel==='menu'&&<div className="modal-body menu-body"><p className="eyebrow">{brand} / COLLECTION</p><h2>Explore.</h2>{categories.map(c=><button key={c} onClick={()=>{setFilter(c);setLimit(12);close();nav('/collection')}}>{c==='All'?'Always On':c}</button>)}<button onClick={()=>{setAccountStart(false);open('account')}}>My Account</button><a className="experience-choice" href="https://iamjohnimah.github.io/spreeai-always-on-demo/experience-selection/"><i aria-hidden="true"/>Choose your experience</a><div className="menu-appearance"><span>Appearance<small>{dark?'Dark':'Light'} mode</small></span><button className="theme-switch" role="switch" aria-label="Dark mode" aria-checked={dark} onClick={toggleTheme}><span/></button></div></div>}
   {panel==='account'&&<ConnectedAccount startWithSignup={accountStart} onDone={close} savedLooks={<Stylist currentProduct={product} voice={voice} person={person} savedOnly embedded onProduct={()=>setPanel(null)} onBag={items=>setBag([...bag,...items])}/>}/>}
