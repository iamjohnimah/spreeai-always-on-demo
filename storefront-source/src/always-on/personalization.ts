...
export type Result={status:'idle'|'loading'|'ready'|'error';url?:string;recommended?:string;map?:FitMap|null;error?:string;fitNote?:string;requestId?:string;environment?:'dev'|'prod';recommendationSource?:'chart'|'engine'|'twin'};
...
function start(key:string,load:()=>Promise<Partial<Result>>,scope:string){if(!key||(cache.has(key)&&!isPresignedUrlExpired(cache.get(key)?.url))||jobs.has(key))return;cache.set(key,{status:'loading'});notify();const finishLoading=beginLoading();const promise=load().then(value=>{if(scope===profileScope()){cache.set(key,{...value,status:'ready'});persistPreviews()}}).catch(e=>{if(scope===profileScope())cache.set(key,{status:'error',error:e instanceof Error?e.message:'Temporarily unavailable.'})}).finally(()=>{finishLoading();jobs.delete(key);notify()});jobs.set(key,promise)}
function useResult(key:string){return useSyncExternalStore(f=>{listeners.add(f);return()=>{listeners.delete(f)}},()=>key?(isPresignedUrlExpired(cache.get(key)?.url)?pending:cache.get(key)||empty):empty)}
export function usePersonalImage(products:Product[],size='',base=''){
 const profile=useConnectedProfile(),identity=profile.identity,scope=profileScope(),ids=products.map(p=>p.garmentId);const usable=identity&&ids.length>0;const key=usable?renderKey(ids,size,base,products[0]?.environment||'dev'):'';const state=useResult(key);
 useEffect(()=>{if(!identity||!ids.length)return;const saved=cache.get(key);const timer=setTimeout(()=>start(key,async()=>{if(scope!==profileScope())throw Error('Profile changed');if(saved?.requestId&&isPresignedUrlExpired(saved.url)){const get=saved.environment==='prod'?productionRequest:request;const refreshed=await get<{image?:{url:string}}>('/v1/user-assets/tryon/'+encodeURIComponent(saved.requestId));if(!refreshed.image?.url)throw Error('This saved preview is temporarily unavailable.');return {...saved,url:refreshed.image.url}}if(products.some(p=>(p.environment||'dev')!==(products[0].environment||'dev')))throw Error('Choose pieces from the same collection for a combined look.');const r=products[0].environment==='prod'?await productionRender(ids,identity,new AbortController().signal,size||undefined,base||undefined):await render(ids,identity,new AbortController().signal,'front',size||undefined,base||undefined);if(scope===profileScope()&&r.image?.url)recordHistory({id:key,kind:size?'sizing':'tryon',identityId:identity.id,identityName:identity.name,pieces:products.map(p=>({id:p.id,name:p.name,image:p.image})),images:[r.image.url],size:size||undefined,recommended:base||undefined});return {url:r.image!.url,requestId:r.request_id,environment:products[0].environment||'dev'}},scope),size||ids.length>1?450:0);return()=>clearTimeout(timer)},[key]);return state;
}
export function usePersonalFit(product?:Product){
 const profile=useConnectedProfile(),identity=profile.identity,scope=profileScope(),key=identity&&product&&!isOneSize(product.sizes)?scope+':'+(product.environment||'dev')+':fit:'+product.garmentId:'';const state=useResult(key);
