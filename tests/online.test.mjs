...
const oldRequire=Module.prototype.require;Module.prototype.require=function(id){return id==='./connection'?{request:async()=>{throw Error('offline')}}:oldRequire.call(this,id)};
const {products,normalizeCatalog}=load('data'),{sdkURL,isSdkClose,SDK_ORIGIN}=load('sdk');
test('catalog has only source-backed garments with original prices and size lists',()=>{assert.equal(products.length,28);assert(products.every(p=>p.garmentId&&p.partnerId&&p.source.length&&p.id==='spree-'+p.garmentId));const p=products[0];assert.equal(p.name,'Crystal Fringe Chandelier Earrings');assert.equal(p.priceLabel,'$720.00');assert(p.sizes.length);assert(!products.some(p=>p.id==='leather-jacket'));});
test('each development try-on uses the selected garment and partner',()=>{for(const p of products){const u=new URL(sdkURL(p.garmentId,p.partnerId));assert.equal(u.origin,'https://vton.dev.spreeai.com');assert.deepEqual(JSON.parse(u.searchParams.get('garments')),[{garmentId:p.garmentId}]);assert.equal(u.searchParams.get('partnerId'),p.partnerId);assert.equal(u.searchParams.get('enableAddToCart'),'false')}});
...
test('bridge close accepts only the exact development frame',()=>{const frame={};assert(isSdkClose(SDK_ORIGIN,frame,frame,{code:'00001'}));assert(!isSdkClose('https://evil.example',frame,frame,{code:'00001'}));assert(!isSdkClose(SDK_ORIGIN,{},frame,{code:'00001'}));assert(!isSdkClose(SDK_ORIGIN,null,null,{code:'00001'}));assert(!isSdkClose(SDK_ORIGIN,frame,frame,{code:'00002'}))});
