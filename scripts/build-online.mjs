...
catalog.push(...JSON.parse(readFileSync(path.join(source,'src/always-on/production-catalog.json'),'utf8')));
const existingProducts=readdirSync(path.join(out,'product'),{withFileTypes:true}).filter(x=>x.isDirectory()).map(x=>'product/'+x.name);
for(const route of new Set(['', 'account','collection',...existingProducts,...catalog.map(p=>'product/spree-'+p.id)])){mkdirSync(path.join(out,route),{recursive:true});copyFileSync(path.join(source,'dist/index.html'),path.join(out,route,'index.html'))}
